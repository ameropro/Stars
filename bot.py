...
FLYER_KEY = "FL-ElIjfC-cUWGPO-SuPtuY-hwPawv"
...
async def get_channel_id(bot: Bot, link: str) -> int | None:
    try:
        username = normalize_channel(link)
        chat = await bot.get_chat(username)
        return chat.id
    except Exception as e:
        logging.error(f"Ошибка получения ID канала {link}: {e}")
        return None

async def need_op(user_id: int, bot: Bot) -> bool:
...
def hash_flyer_task(signature: str, user_id: int) -> str:
    """
    Уникальный хеш задания для конкретного пользователя
    """
    raw = f"{signature}:{user_id}"
    return hashlib.sha256(raw.encode()).hexdigest()

async def resolve_channel_id(bot: Bot, link: str):
    try:
        username = link.replace("https://t.me/", "").replace("http://t.me/", "")
        chat = await bot.get_chat(username)
        return chat.id
    except Exception as e:
        logging.error(f"Не удалось получить ID канала {link}: {e}")
        return None

async def show_next_flyer_task(user_id: int, bot: Bot, language_code: str = "ru"):
    ...
    if not selected_task:
        await bot.send_message(
            user_id,
            "<b>🎉 Вы выполнили все доступные задания!</b>",
            parse_mode="HTML"
        )
        return
    ...
    link = (
        selected_task.get("url")
        or selected_task.get("link")
        or (
            selected_task.get("links")[0]
            if isinstance(selected_task.get("links"), list) and selected_task.get("links")
            else None
        )
    )

    channel_name = selected_task.get("name", "канал")

    if not link:
        await bot.send_message(
            user_id,
            "⚠️ Следующее задание недоступно.",
            parse_mode="HTML"
        )
        return
    ...
    text = (
        "<b>✨ Новое задание ✨</b>\n\n"
        f"📢 Подпишитесь на канал: <b>{channel_name}</b>\n\n"
        f"🎁 Награда: <b>{task_grant[0]} ⭐</b>\n\n"
        "После подписки нажмите <b>«Проверить»</b>."
    )

    markup = InlineKeyboardBuilder()
    markup.button(text="✅ Подписаться", url=link)
    markup.button(text="🔎 Проверить", callback_data=f"flyer_check:{signature}")
...
async def check_op_callback(call: CallbackQuery, bot: Bot):
    ...
    for sponsor_id in get_all_id_opnotcheck():
        ...
        if not await is_user_subscribed(bot, user_id, sponsor_id):
            await call.answer(
                "❌ Вы подписались не на всех спонсоров",
                show_alert=True
            )
            return

        if not has_user_checked(user_id, sponsor_id):
...
async def check_op_callback(call: CallbackQuery, bot: Bot):
    ...
    for sponsor_id in get_all_id_opnotcheck():
        ...
        # ❗ РЕАЛЬНАЯ ПРОВЕРКА ЧЕРЕЗ TELEGRAM
        if not await is_user_subscribed(bot, user_id, sponsor_id):
            await call.answer(
                "❌ Вы подписались не на всех спонсоров",
                show_alert=True
...
async def subbed_op_notchek_callback(callback: CallbackQuery, bot: Bot):
        ...
        if balance >= bet:
            ...
            if random.random() < 0.30:
                ...
                if coefficient > 0:
                    ...
                    input_photo_game = FSInputFile("mini_game.jpg")
                    ...
                    builder_game.button(text="Назад в меню мини-игр", callback_data="mini_games")
                    markup_game = builder_game.adjust(3, 3, 1).as_markup()
                    input_photo_game_lose = FSInputFile("mini_game.jpg")
                    await bot.delete_message(chat_id=user_id, message_id=call.message.message_id)
                ...
                builder_game.button(text="Назад в меню мини-игр", callback_data="mini_games")
                markup_game = builder_game.adjust(3, 3, 1).as_markup()
                input_photo_game_no_luck = FSInputFile("mini_game.jpg")
                await bot.delete_message(chat_id=user_id, message_id=call.message.message_id)
...
async def mini_games_callback(call: CallbackQuery, bot: Bot):
    ...
    with open('mini_game.jpg', 'rb') as photo:
        input_photo_minigames = FSInputFile("mini_game.jpg")
...
async def play_game_callback(call: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        with open('mini_game.jpg', 'rb') as photo:
            input_photo_playgame = FSInputFile("mini_game.jpg")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url,
                headers=headers,
                data=payload   # ❗ ВАЖНО: data, НЕ json
            ) as response:

                text = await response.text()
                print("FLYER get_tasks RAW:", text)  # DEBUG

                if response.status != 200:
                    logging.error(f"Flyer error {response.status}: {text}")
                    return None
        ...
        logging.exception("Ошибка Flyer get_tasks", e)
        return None
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                url,
                headers=headers,
                data=payload   # ❗ тоже data
            ) as response:

                text = await response.text()
                print("FLYER check_task RAW:", text)  # DEBUG

                if response.status != 200:
                    logging.error(f"Flyer check error {response.status}: {text}")
                    return None
        ...
        logging.exception("Ошибка Flyer check_task", e)
        return None
...
async def tasks_callback(call: CallbackQuery, bot: Bot):
    ...
    await show_next_flyer_task(
        user_id=user_id,
        bot=bot,
        language_code=call.from_user.language_code or "ru"
    )

@router.callback_query(F.data.startswith("flyer_click:"))
async def flyer_click_callback(call: CallbackQuery, bot: Bot):
    user_id = call.from_user.id
    ...
    if is_flyer_task_completed(task_hash):
        await call.answer("🎯 Задание уже выполнено", show_alert=True)
        return
    ...
    # ▶️ показываем следующее задание
    await show_next_flyer_task(user_id, bot)
...
async def flyer_check_callback(call: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        # 🔎 Проверка через Flyer
        result = await flyer.check_task(
            user_id=user_id,
            signature=signature
        )

        if result in ("complete", "waiting"):
            # ⭐ Награда
            increment_stars(user_id, task_grant[0])
            add_flyer_task(task_hash)
        ...
        logging.exception(f"Ошибка flyer_check_callback: {e}")
        await bot.answer_callback_query(
            call.id,
            "⚠️ Ошибка проверки задания",
            show_alert=True
        )
...
async def withdraw_stars_menu_callback(call: CallbackQuery, bot: Bot):
    ...
    builder_stars.button(text="100 ⭐️(💍)", callback_data="withdraw:100:💍")
    builder_stars.button(text="100 ⭐️(💎)", callback_data="withdraw:100:💎")
    builder_stars.button(text="Telegram Premium 1мес. (1000 ⭐️)", callback_data="withdraw:premium1")
    builder_stars.button(text="Telegram Premium 3мес. (1500 ⭐️)", callback_data="withdraw:premium2")
    ...
    try:
        ...
        with open('withdraw_stars.jpg', 'rb') as photo:
            input_photo_withdraw = FSInputFile("withdraw_stars.jpg")
...
async def my_balance_callback(call: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        with open('profile.jpg', 'rb') as photo:
            input_photo_profile = FSInputFile("profile.jpg")
...
async def promocode_callback_query(call: CallbackQuery, bot: Bot, state: FSMContext):
    await bot.delete_message(call.from_user.id, call.message.message_id)
    with open('promocode.jpg', 'rb') as photo:
        input_photo_promo = FSInputFile("promocode.jpg")
...
async def faq_callback(call: CallbackQuery, bot: Bot):
    ...
    guide_photo = FSInputFile("guide.jpg")
...
- <b>3 уровень:</b> {nac_3[0] * 2 if user_is_booster else nac_3[0]} звезды ⭐️ (250+ приглашений)
    ...
    with open("get_url.jpg", "rb") as photo:
        input_photo_earn = FSInputFile("get_url.jpg")
...
async def back_main_callback(call: CallbackQuery, bot: Bot):
    ...
    try:
        ...
        with open('start.jpg', 'rb') as photo:
            input_photo_back_main = FSInputFile("start.jpg")
...
async def show_leaderboard(message: Message, period, bot: Bot):
    ...
    try:
        ...
        with open('leaders.jpg', 'rb') as photo:
            input_photo_leaders = FSInputFile("leaders.jpg")
